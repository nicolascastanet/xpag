...
from gymnasium.vector.utils import write_to_shared_memory, concatenate, create_empty_array
...
def gymnasium_vec_env_(env_name, num_envs, wrap_function=None):
    ...
    if "num_envs" in inspect.signature(
        ...
        env_type = (
            ...
            "Fetch"
            if hasattr(dummy_env.unwrapped, "initial_qvel")
            and hasattr(dummy_env.unwrapped, "initial_qvel")
...
def gymnasium_vec_env(env_name: str, num_envs: int, wrap_function: Callable = None):
    env, env_info, dummy_env = gymnasium_vec_env_(env_name, num_envs, wrap_function)
    eval_env, _, _ = gymnasium_vec_env_(env_name, 1, wrap_function)
